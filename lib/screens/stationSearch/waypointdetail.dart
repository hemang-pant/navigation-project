...
class WaypointDetails extends StatefulWidget {
  ...
  @override
  _WaypointDetailsState createState() => _WaypointDetailsState();
}
...
class _WaypointDetailsState extends State<WaypointDetails> {
  ...
  Future<Map<String, dynamic>> getDirections(
      Marker origin, Marker waypoint, Marker destination) async {
    instructionText.remove('');
    String key = 'AIzaSyCOzoP4mN1S1DuNtDbjOwjRlbSimSSEt10';
    String url =
        'https://maps.googleapis.com/maps/api/directions/json?origin=${origin.position.latitude},${origin.position.longitude}&destination=${waypoint.position.latitude},${waypoint.position.longitude}&key=$key';
    log("making request");
    log("url: $url");
    ...
    url =
        'https://maps.googleapis.com/maps/api/directions/json?origin=${waypoint.position.latitude},${waypoint.position.longitude}&destination=${destination.position.latitude},${destination.position.longitude}&key=$key';
    ...
    results2['bounds_ne'] = results['bounds_ne'] + results2['bounds_ne'];
    results2['bounds_sw'] = results['bounds_sw'] + results2['bounds_sw'];
    results2['start_location'] =
        results['start_location'] + results2['start_location'];
    results2['end_location'] =
        results['end_location'] + results2['end_location'];
    results2['polyline'] = results['polyline'] + results2['polyline'];
    results2['instruction'] = results['instruction'] + results2['instruction'];
    results2['polyline_decoded'] =
        results['polyline_decoded'] + results2['polyline_decoded'];
          ...
          Column(
            children: [
              ...
              Padding(
                ...
                child: Text(widget.destination.markerId.value,
              ...
              Text(
                  '${(Geolocator.distanceBetween(origin.position.latitude, origin.position.longitude, widget.destination.position.latitude, widget.destination.position.longitude) / 1000).toStringAsFixed(2)}KM Away from you',
                  style: GoogleFonts.montserrat(
                      fontSize: 20,
                      fontWeight: FontWeight.bold,
                      color: Colors.black)),
                  ...
                  onPressed: () async {
                    Uri googleUrl = Uri.parse(
                        'https://www.google.com/maps/dir/?api=1&origin=' +
                            ...
                            '&travelmode=driving' +
                            '&waypoints=' +
                            widget.waypoint.position.latitude.toString() +
                            ',' +
                            widget.waypoint.position.longitude.toString() +
                            '');
